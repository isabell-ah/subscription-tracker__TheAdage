...
<div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 0; background-color: #f4f7fa;">
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color:rgb(225, 244, 214); border-radius: 10px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        <tr>
            <td style="background-color:rgb(48, 110, 36); text-align: center;">
                <p style="font-size: 54px; line-height: 54px; font-weight: 800;">TheAdage Hive</p>
            </td>
        </tr>
        <tr>
            <td style="padding: 40px 30px;">                
                ...
                ${
                  sleepMessage
                    ? `<p style="font-size: 16px; margin-bottom: 25px; background-color: #f0f7ff; padding: 15px; border-radius: 8px;"><strong>System Message:</strong> ${sleepMessage}</p>`
                    : ''
                }
                
                ${
                  scheduledReminders
                    ? `<div style="font-size: 16px; margin-bottom: 25px; background-color: #f0f7ff; padding: 15px; border-radius: 8px;">
                    <strong>Scheduled Reminders:</strong><br>
                    ${scheduledReminders}
                </div>`
                    : ''
                }
        ...
        <tr>
            <td style="background-color: #f0f7ff; padding: 20px; text-align: center; font-size: 14px;">
                <p style="margin: 0 0 10px;">
                   TheAdage  Inc. | 123 TM St, Nairobi, KE 12345
                </p>
                <p style="margin: 0;">
...
const emailTemplates = [
  {
    ...
    generateSubject: (data) =>
      `📅 Reminder: Your ${data.subscriptionName} Subscription Renews in ${data.daysBefore} Days!`,
    generateBody: (data) =>
      generateEmailTemplate({
        ...data,
        daysLeft: data.daysBefore || 7,
  ...
  },
  {
    ...
    generateSubject: (data) =>
      `🔔 Subscription Renewal Reminders Scheduled for ${data.subscriptionName}`,
    generateBody: (data) =>
      generateEmailTemplate({
        ...data,
        daysLeft: '(multiple reminders scheduled)',
